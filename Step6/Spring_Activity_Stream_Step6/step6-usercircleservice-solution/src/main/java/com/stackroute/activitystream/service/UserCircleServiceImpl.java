...
public class UserCircleServiceImpl implements UserCircleService {
	 ...
	 */
	/*
	 * public boolean addUser(String username, String circleName) { Boolean flag =
	 * true;
	 * 
	 * if (circleRepository.exists(circleName) && userRepository.exists(username)) {
	 * if (get(username, circleName) == null) { UserCircle userCircle = new
	 * UserCircle(); userCircle.setUsername(username);
	 * userCircle.setCircleName(circleName); userCircleRepository.save(userCircle);
	 * flag = true; } else { flag = false; } } else { flag = false; } return flag; }
	 */
