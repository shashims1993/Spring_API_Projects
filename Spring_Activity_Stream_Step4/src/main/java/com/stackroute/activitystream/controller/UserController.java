...
public class UserController {
	 ...
	 * created 2. (CONFLICT) - If the username conflicts with any existing user
	 * 
	...
	public ResponseEntity<?> createUser(@RequestBody User user) {
		...
		User userObj = userDAO.get(user.getUsername());
              if (userObj != null) {
	 ...
	 * is not found 3. (UNAUTHORIZED) - If the user trying to perform the action has
	 * not logged in
	 * 
	...
	@PutMapping(path = "{username}")
	public ResponseEntity<?> updateUser(@PathVariable("username") String username , @RequestBody User user, HttpSession session) {
		if (session.getAttribute("LoginUserName") != null) {
