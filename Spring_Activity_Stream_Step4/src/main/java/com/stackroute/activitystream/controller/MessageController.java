...
public class MessageController {
	...
	public ResponseEntity<?> sendMessageToCircle(@PathVariable("circleName") String circleName,
		...
		flag = messageDAO.sendMessageToCircle(circleName, message);
		if (flag) {
	...
	public ResponseEntity<?> sendMessageToUser(@PathVariable("receiverId") String receiverId,
		...
		message.setSenderName(logginUserName);
		flag = messageDAO.sendMessageToUser(receiverId, message);
		if (flag) {
	...
	public ResponseEntity<?> retriveMessageFromUser(@PathVariable("senderUsername") String senderUsername,
		...
		List<Message> list = messageDAO.getMessagesFromUser(senderUsername, receiverUserName, pageNumber);
		if (list.size() > 0) {
	...
	public ResponseEntity<?> retriveMessageFromCircle(@PathVariable("circleName") String circleName,
		...
		List<Message> list = messageDAO.getMessagesFromCircle(circleName, pageNumber);
		if (list.size() > 0) {
			return new ResponseEntity<List<Message>>(list, HttpStatus.OK);
		} else {
			return new ResponseEntity<String>("No message found", HttpStatus.NOT_FOUND);
	...
	public ResponseEntity<?> retriveAllTags(HttpSession session) {
		...
		List<String> list = messageDAO.listTags();
		if (list.size() > 0) {
	...
	public ResponseEntity<?> retriveAllTagsByUser(@PathVariable("tag") String tag,
		...
		List<Message> list = messageDAO.showMessagesWithTag(tag, pageNumber);
		if (list.size() > 0) {
			return new ResponseEntity<List<Message>>(list, HttpStatus.OK);
		} else {
	...
	public ResponseEntity<?> subscribeUserToTag(@PathVariable("username") String username, @PathVariable("tag") String tag,
			HttpSession session) {
		String logginUserName = (String) session.getAttribute("LoginUserName");
		Boolean flag = false;
		if (logginUserName == null) {
		...
		flag = messageDAO.subscribeUserToTag(username, tag);
		if (flag) {
	...
	public ResponseEntity<?> unsubscribeUserToTag(@PathVariable("username") String username, @PathVariable("tag") String tag,
			HttpSession session) {
		String logginUserName = (String) session.getAttribute("LoginUserName");
		Boolean flag = false;
		if (logginUserName == null) {
		...
		flag = messageDAO.unsubscribeUserToTag(username, tag);
		if (flag) {
	...
	public ResponseEntity<?> retriveAllTagsByUser(@PathVariable("username") String username, HttpSession session) {
		...
		List<String> list = messageDAO.listMyTags(username);
		if (list.size() > 0) {
			return new ResponseEntity<List<String>>(list, HttpStatus.OK);
		} else {
			return new ResponseEntity<String>("No tags found", HttpStatus.NOT_FOUND);
